...
## Beispiel für das Anlegen von hierarchischen Vokabularen für Sammlungen

```bash
HOST=localhost PORT=8081 TOKEN=secret vocabulary-init-script/install.sh hierarchical_collections
```

## Vollständige CURL Aufrufe
Für jedes Beispiel lassen sich die konkreten CURL Aufrufe auch einfach anzeigen:
```bash
VERBOSE=1 HOST=localhost PORT=8081 TOKEN=secret vocabulary-init-script/install.sh collections
```

Das sieht im Beispiel für Sammlungen so aus:
```bash
...
Oder im Fall vom komplexeren Bücher Beispiel dann so:
```bash
# === Sprachen ===
